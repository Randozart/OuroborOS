...
| Plan 9 (Bell Labs, 1985-2002; maintained as 9front) | 1985-2002 | Art. 1/3/5's distributed ancestor: **9P uniform resource protocol**, per-process **name spaces**, no local/remote (`cpu`, `import`/`exportfs`), identity via factotum | We replace the byte-file with the typed, priced, revocable graph edge (Art. 3/7); name spaces become per-placement bound *views* re-verified by parity contracts (Art. 10); bulk never rides the control protocol (Art. 6); our deltas numbered in docs/PLAN9.md §2.1 |
| SwarmLLM (N. Narendrula, MIT) | 2026 | Art. 6/7 evidence: the **air path is a protocol problem, not a penalty** — batched prefill + MTP speculation + exact rollback move 3-7 tokens per slow lap; memory-roofline WebGPU decode (183/184 GB/s) | We keep the slow-link protocol but invert its carrier: no WebRTC room, no mesh — one graph, one arbiter (Art. 3); wired + air **bonded as priced, revocable lanes** (Art. 4/6/7); deltas in docs/AIR_PATH.md §2.3 |
